...
import static com.foodymoody.be.docs.member.MemberSteps.보노의_회원비밀번호를_수정한다;
...
import static com.foodymoody.be.docs.member.MemberSteps.응답코드가_200이고_응답에_id가_존재하며_보노의_회원프로필이_조회되는지_검증한다;
import static com.foodymoody.be.docs.member.MemberSteps.응답코드가_204이고_보노가_수정_전의_비밀번호로_로그인에_실패하는지_검증한다;
...
class MemberAcceptanceTest extends Document {
    ...
    @DisplayName("회원 가입 성공하면, 응답코드 200과 id를 반환하고 회원프로필이 조회된다.")
    @Test
    void when_registerMember_then_response200AndId_and_canLoadMemberProfile() {
        ...
        // then
        응답코드가_200이고_응답에_id가_존재하며_보노의_회원프로필이_조회되는지_검증한다(response);
    }

    @DisplayName("회원 탈퇴 성공하면, 응답코드 204를 반환하고 회원프로필이 조회되지 않는다.")
    @Test
    ...
    void when_updateMemberPassword_then_response204_and_failLoginByOriginalPassword() {
        ...
        // when
        var response = 보노의_회원비밀번호를_수정한다(spec);

        // then
        응답코드가_204이고_보노가_수정_전의_비밀번호로_로그인에_실패하는지_검증한다(response);
    }

    @DisplayName("회원 프로필 조회 성공하면, 응답코드 200과 회원프로필을 반환한다.")
    @Test
