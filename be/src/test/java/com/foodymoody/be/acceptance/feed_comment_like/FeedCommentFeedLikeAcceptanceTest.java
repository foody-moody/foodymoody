...
import static com.foodymoody.be.acceptance.feed_comment_like.FeedCommentLikeSteps.댓글에_좋아요를_취소한다;
...
class FeedCommentFeedLikeAcceptanceTest extends AcceptanceTest {
    ...
    void createCommentHeart() {
        ...
        // when
        var response = 댓글에_좋아요를_누른다(commentId, feedId, 회원푸반_액세스토큰, spec);
    ...
    void deleteCommentHeart() {
        ...
        // given
        댓글에_좋아요를_누른다(commentId, feedId, 회원푸반_액세스토큰, new RequestSpecBuilder().build());

        // when
        var response = 댓글에_좋아요를_취소한다(commentId, feedId, 회원푸반_액세스토큰, spec);
