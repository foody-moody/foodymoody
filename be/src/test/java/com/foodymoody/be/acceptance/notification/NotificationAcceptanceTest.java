...
class NotificationAcceptanceTest extends AcceptanceTest {
    ...
    void setUp() {
        ...
        댓글에_좋아요를_누른다(회원푸반_액세스토큰, commentId, spec);

        String replyId = 댓글에_댓글을_등록하고_아이디를_가져온다(feedId, commentId, 회원아티_액세스토큰);
        대댓글에_좋아요를_추가한다(회원푸반_액세스토큰, commentId, replyId);
        ...
        피드_컬렉션_댓글에_좋아요를_등록한다(회원푸반_액세스토큰, feedCollectionCommentId);

        피드_컬렉션_댓글에_대댓글을_등록한다(회원푸반_액세스토큰, feedCollectionCommentId);

        String feedCollectionReplyId = 피드_컬렉션_댓글에_대댓글을_등록하고_아이디를_반환한다(회원아티_액세스토큰, feedCollectionCommentId);
        피드_컬렉션_대댓글에_좋아요를_등록한다(회원푸반_액세스토큰, feedCollectionCommentId, feedCollectionReplyId);
    }
