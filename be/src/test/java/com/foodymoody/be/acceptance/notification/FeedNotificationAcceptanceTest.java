...
import static com.foodymoody.be.acceptance.notification.NotificationSteps.모든_알람을_읽음으로_변경한다;
...
import static com.foodymoody.be.acceptance.notification.NotificationSteps.유저의_모든_읽음_알람을_삭제한다;
...
class FeedNotificationAcceptanceTest extends AcceptanceTest {
    ...
    @DisplayName("모든 읽음 알람을 모두 삭제하면 응답코드 204를 반환한다")
    @Test
    void when_delete_all_read_if_success_then_return_code_204() {
        ...
        // when
        var response = 유저의_모든_읽음_알람을_삭제한다(회원아티_액세스토큰, spec);
    ...
    class ExistsNotifications {
        ...
        void when_request_mark_all_read_if_success_return_code_204() {
            ...
            // when
            var response = 모든_알람을_읽음으로_변경한다(회원아티_액세스토큰, spec);
